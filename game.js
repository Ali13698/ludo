...
// ============ CONFIG ============
var cfg = { players: 2 };
var optBtns = document.querySelectorAll('#optPlayers button');
for (var bi = 0; bi < optBtns.length; bi++) {
  (function(btn) {
    btn.onclick = function() {
      for (var j = 0; j < optBtns.length; j++) optBtns[j].className = '';
      this.className = 'on';
      cfg.players = parseInt(this.getAttribute('data-v'), 10);
    };
  })(optBtns[bi]);
}
...
if (window.ResizeObserver) {
  var ro = new ResizeObserver(function() {
    if (boardArea.clientWidth > 0 && boardArea.clientHeight > 0) resize();
  });
  ro.observe(boardArea);
}
window.addEventListener('resize', function() {
  if (boardArea.clientWidth > 0) resize();
});
...
function startMove(t, d, cb) {
  ...
  t.wps = [];
  if (from === 0) {
    t.wps.push(gridOf(t.player, 1, t.idx));
  } else {
    for (var p = from + 1; p <= to; p++) t.wps.push(gridOf(t.player, p, t.idx));
  }
  t.moving = true;
  var speed = 0.15;
  function step() {
...
function checkWin() {
  for (var a = 0; a < active.length; a++) {
    var p = active[a], c = 0;
    for (var i = 0; i < tokens.length; i++) {
      if (tokens[i].player === p && tokens[i].pos === 57) c++;
    }
    if (c >= 4) return p;
  }
  return -1;
  ...
  timerInterval = setInterval(function() {
    ...
    if (timeLeft <= 0) {
      ...
      if (cur === active[myIdx] && !outMap[cur]) {
        ...
        if (strikeMap[cur] >= 2) {
          outMap[cur] = true;
          statusText.textContent = P[cur].name + ' اوت شد!';
          renderProfiles();
...
function settled() {
  ...
  if (cur !== active[myIdx]) {
    // ربات
    setTimeout(function() { botMove(mv); }, 500);
  } else if (mv.length === 1) {
...
function doMove(t) {
  ...
  startMove(t, d, function() {
    ...
    if (w >= 0) {
      over = true;
      sWin();
      statusText.textContent = '🏆 برنده: ' + P[w].name;
...
function botMove(mv) {
  // ربات بهترین حرکت رو انتخاب می‌کنه
...
function nextTurn() {
  ...
  if (cur === active[myIdx]) {
    ...
    botTimeout = setTimeout(botTurn, 800);
  }
}
...
function updateUI() {
  ...
  if (cur === active[myIdx] && !outMap[cur]) {
    ...
    if (outMap[cur]) statusText.textContent = P[cur].name + ' اوت شده';
    else statusText.textContent = 'نوبت ' + P[cur].name + '...';
  }
}
...
function renderProfiles() {
  ...
  for (var i = 0; i < active.length; i++) {
    ...
    nm.textContent = (p === active[myIdx]) ? 'شما' : P[p].name;
...
function newGame() {
  active = [];
  var order = [0, 1, 3, 2];
  for (var i = 0; i < cfg.players; i++) active.push(order[i]);
  initTokens();
  cur = active[0];
  dice = 0;
  rolled = false;
  ...
  for (var j = 0; j < active.length; j++) {
    strikeMap[active[j]] = 0;
    outMap[active[j]] = false;
  }
  diceBox.textContent = '🎲';
  updateUI();
}
