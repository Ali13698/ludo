window.Game = {
  ...
  doRoll: function(){
    ...
    if (S.isOnline && S.cur !== S.myIdx) return; // نوبت حریف
    Timer.clear();
    SFX.dice();

    // در حالت آنلاین، عدد تاس رو خودمون تعیین می‌کنیم و به سرور می‌فرستیم
  ...
  settled: function(){
    ...
    // اگه حرکتی ممکنه و نوبت ماست
    if (S.cur === S.active[S.myIdx] || !S.isOnline){
      // اگه رباته یا فقط 1 حرکت ممکنه → خودکار
      ...
      // اگه آنلاین و نوبت ماست، منتظر کلیک کاربر می‌مونیم
    }
  },
  ...
  doMove: function(t){
    ...
    // در آنلاین، اگه نوبت ما نیست، اجازه نداریم
    ...
    // ارسال حرکت به سرور (قبل از شروع انیمیشن)
    if (S.isOnline && window.Online && Online.ws){
    ...
    Move.start(t, d, function(){
      ...
      if (w >= 0){
        ...
        const name = (w === S.active[S.myIdx]) ? 'شما' :
                     (S.isOnline ? (S.botInfo[w] ? S.botInfo[w].name : 'حریف') :
  ...
  // ============ ONLINE: دریافت حرکت حریف ============
  applyOpponentMove: function(data){
    // data: { player, idx, from, to }
