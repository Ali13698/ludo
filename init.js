...
function setupOnlineEvents(){
  ...
  Online.on('game_action', (msg) => {
    const d = msg.data;
    if (!d) return;
    if (window.BotOpponent && BotOpponent.active) return;
...
function startOnlineGame(){
  ...
  if (Online.opponent){
    S.botInfo[oppIdx] = { name: Online.opponent.name || 'حریف', avatar: Online.opponent.avatar || '🎮' };
  }
  S.tokens = [];
  for (let p = 0; p < 2; p++){
    for (let i = 0; i < 4; i++){
  ...
  if (Online.opponent && Online.opponent.isBot){
    BotOpponent.start(Online.opponent);
  } else {
    BotOpponent.stop();
  }
    ...
    requestAnimationFrame(() => {
      ...
      if (S.cur === S.myIdx) Timer.start();
        ...
        if (window.BotOpponent && BotOpponent.active){
          setTimeout(() => BotOpponent.playTurn(), 1000);
        }
      }
    });
  });
...
function endOnlineGame(){
  ...
  if (window.Online && Online.ws && Online.ws.readyState === 1){
    Online.send({ type: 'leave_room' });
  }
...
function setupInputHandlers(){
  ...
  DOM.canvas.onclick = function(e){
    ...
    const gx = (e.clientX - rect.left) / S.CELL;
    const gy = (e.clientY - rect.top) / S.CELL;
    const mv = Rules.getMovable();
    let best = null, bd = 0.8;
    for (let i = 0; i < mv.length; i++){
      const t = mv[i];
      const g = Rules.gridOf(t.player, t.pos, t.idx);
      const dx = g[0] - gx, dy = g[1] - gy;
  ...
  if (gameMenuBtn) gameMenuBtn.onclick = function(){
    if (window.GameSettings) GameSettings.open();
  };
}
