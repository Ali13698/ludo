...
window.onerror = function(m, s, l){
  // خطای cross-origin تلگرام رو نادیده بگیر
...
function setupOnlineEvents(){
  ...
  // دریافت حرکت/تاس حریف انسانی
  Online.on('game_action', (msg) => {
    const d = msg.data;
    if (!d) return;
    // اگه حریف رباته، حرکت از سرور نمیاد (خود کلاینت مدیریت می‌کنه)
...
function startOnlineGame(){
  ...
  // ⭐ اگه حریف رباته، BotOpponent رو شروع کن
  if (Online.opponent && Online.opponent.isBot){
    BotOpponent.start(Online.opponent);
    console.log('[online] opponent is BOT:', Online.opponent.name);
  } else {
    BotOpponent.stop();
  }
